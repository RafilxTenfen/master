...
fn main() -> Result<()> {
  ...
  pcap_process(rtshark, &conn, &mut map_id, &mut map_attacks);
...
pub fn pcap_process(
  ...
  while let Some(packet) = rtshark.read().unwrap_or_else(|e| {
    ...
    add_packet_to_attacks(map_attacks, packet);
 }
}

pub fn add_packet_to_attacks(cidr_udp_attack: &mut HashMap<Ipv4Cidr, HashMap<i32, PcapAttack>>, packet: pcap_packet::PcapPacket) {
  ...
  match cidr_udp_attack.get_mut(&cidr) {
    Some(udp_attack) => {
      ...
      match udp_attack.get_mut(&udp_dest_port) {
        ...
        None => {
          // no attack
          udp_attack.insert(udp_dest_port, new_empty_attack(packet));
        },
      }
    },
    ...
    None => {
      // não tem o cidr,
      let udp_attack = new_map_attack(packet);
      cidr_udp_attack.insert(cidr, udp_attack);
    },
  }
}

pub fn new_map_attack(packet: pcap_packet::PcapPacket) -> HashMap<i32, PcapAttack> {
  let mut map_attack = HashMap::<i32, PcapAttack>::new();
  map_attack.insert(packet.udp.destination_port, new_empty_attack(packet));

  return map_attack;
}

pub fn new_empty_attack(packet: pcap_packet::PcapPacket) -> PcapAttack {
