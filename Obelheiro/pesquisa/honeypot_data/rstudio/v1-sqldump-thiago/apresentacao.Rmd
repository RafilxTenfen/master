...
dns_plot_logevidade_meses = data_dns_grouped %>%
  ...
  theme(plot.margin = margin(r = 15), plot.title = element_text(size = 22), axis.title = element_text(size = 16))
...
data_dns_grouped['order'] = 1:16125

data_dns_grouped %>%
  filter(grepl('dfafa', qname))
...
data_dns_grouped %>%
  filter(tempo_diff_secs > (months * 9)) 
